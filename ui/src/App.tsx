...
const Icons = {
  ...
  Extract: () => (
    <svg
      ...
      strokeLinejoin="round"
    >
      <path d="M12 3v18M3 12h18M5 19 19 5" />
    </svg>
  ),
  Config: () => (
    <svg
  ...
  Explorer: () => (
    ...
    >
      ...
      <path d="m16.2 7.8-2 2-2 2-2 2 4-4Z" />
...
export default function App() {
  const [activeTab, setActiveTab] = useState<
    "dashboard" | "extract" | "config" | "explorer"
  >("dashboard");
  ...
  const loadKADetails = async (path: string) => {
    try {
      const res = await fetch(
        `${API_BASE}/api/ka/data?path=${encodeURIComponent(path)}`,
      );
      ...
      nodesList.forEach((n: any) => {
        if (n.time) timesSet.add(String(n.time));
      });
      edgesList.forEach((e: any) => {
        if (e.time) timesSet.add(String(e.time));
      });
      ...
      const isHg =
        data.metadata?.template?.includes("hypergraph") ||
        data.metadata?.template?.includes("narrative");
      setHypergraphMode(!!isHg);

      // Auto-detect spatial
      const isSp =
        data.metadata?.template?.includes("spatial") ||
        data.metadata?.template?.includes("map");
  ...
  const updateAgent = async (updated: Partial<AppConfig["agent"]>) => {
    try {
  ...
  const handleRemove = async (path: string) => {
    if (
      !confirm(
        "Are you sure you want to remove this Knowledge Abstract from the UI directory? (Your files will NOT be deleted)",
      )
    )
      return;
    try {
      const res = await fetch(
        `${API_BASE}/api/ka/remove?path=${encodeURIComponent(path)}`,
        {
          method: "DELETE",
        },
      );
  ...
  const triggerExtraction = async (e: React.FormEvent) => {
    e.preventDefault();
    if (extractionForm.inputType === "text" && !extractionForm.inputText) {
      alert("Please enter document text to extract knowledge.");
      return;
    }
    if (extractionForm.inputType === "path" && !extractionForm.inputPath) {
      alert("Please enter an absolute path to extract knowledge from.");
      return;
    }
    ...
    try {
      const res = await fetch(`${API_BASE}/api/ka/parse`, {
        ...
        body: JSON.stringify({
          input_text:
            extractionForm.inputType === "text"
              ? extractionForm.inputText
              : undefined,
          input_path:
            extractionForm.inputType === "path"
              ? extractionForm.inputPath
              : undefined,
  ...
  const triggerFeed = async () => {
    ...
    // Check if user wants to use a file path or paste text
    const isPathMode = window.confirm(
      "Do you want to append from a local file/directory path? (Click OK for Path, Cancel for Pasted Text)",
    );
    ...
    if (isPathMode) {
      pathStr = prompt("Enter absolute directory or file path:") || "";
      if (!pathStr) return;
    } else {
      textStr =
        prompt("Enter document content to append incrementally to this KA:") ||
        "";
      if (!textStr) return;
    }
    ...
    try {
      const res = await fetch(`${API_BASE}/api/ka/feed`, {
        ...
        body: JSON.stringify({
          path: activeKA.path,
          input_text: textStr ? textStr : undefined,
          input_path: pathStr ? pathStr : undefined,
        }),
      });
  ...
  const sendAgentChatMessage = async (e: React.FormEvent) => {
    e.preventDefault();
    ...
    try {
      const res = await fetch(`${API_BASE}/api/agent/talk`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          query: userMsg,
  ...
  const filteredTemplates = templates.filter((t) => {
    const qMatch =
      ...
      t.description_zh
        .toLowerCase()
        .includes(templateFilter.query.toLowerCase()) ||
      t.description_en
        .toLowerCase()
        .includes(templateFilter.query.toLowerCase());

    const domMatch =
      templateFilter.domain === "all" || t.id.startsWith(templateFilter.domain);
    const typeMatch =
      templateFilter.type === "all" || t.type === templateFilter.type;
          ...
          >
            ...
            >
              <div>
                ...
                <p style={{ color: "var(--text-muted)" }}>
                  Manage extracted knowledge abstracts and import directories.
                </p>
              </div>
            ...
            <div className="glass-panel" style={{ padding: "24px" }}>
              <h3 style={{ fontSize: "18px", marginBottom: "16px" }}>
                Registered Knowledge Abstracts
              </h3>
              ...
              {kas.length === 0 ? (
                ...
                >
                  No Knowledge Abstracts found. Run extraction or register a
                  directory to begin.
                </div>
              ) : (
                <div
                      ...
                      >
                        <div style={{ textAlign: "right" }}>
                          ...
                          {ka.updated_at && (
                            ...
                            >
                              Updated{" "}
                              {new Date(ka.updated_at).toLocaleDateString()}
                            </div>
                          )}
                        </div>
          ...
          >
            ...
            >
              <div>
                ...
                <p style={{ color: "var(--text-muted)" }}>
                  Extract structured graphs or models from unstructured
              ...
              {activeTask && (
                ...
                >
                  ...
                  <LogConsole
                    ...
                    height="280px"
                    isCompleted={activeTask.status !== "running"}
                  />
                </div>
              )}

              {/* Real-time Streaming Graph Display */}
              {activeTask &&
                activeTask.status === "running" &&
                activeTask.partial_data && (
                  <div
                    className="glass-panel"
                    style={{
                      height: "500px",
                  ...
                  >
                    ...
                    >
                      ...
                      >
                        {activeTask.partial_data.nodes?.length || 0} nodes ·{" "}
                        {activeTask.partial_data.edges?.length || 0} edges
                      </div>
                    </div>
                    <div style={{ flexGrow: 1, position: "relative" }}>
                      <ForceGraph
                        nodes={activeTask.partial_data.nodes || []}
                        edges={activeTask.partial_data.edges || []}
                        isHypergraph={activeTask.template?.includes(
                          "hypergraph",
                        )}
                        isSpatial={
                          activeTask.template?.includes("spatial") ||
                          activeTask.template?.includes("map")
                        }
                      />
                    </div>
                  </div>
                ...
                >
                  ...
                  >
                    ...
                    {extractionForm.inputType === "text" ? (
                      ...
                      <input
                        type="text"
                        placeholder="Absolute path to a file or directory (e.g. /home/user/data or C:\data.txt)"
                        value={extractionForm.inputPath}
                        onChange={(e) =>
                          setExtractionForm((prev) => ({
                            ...prev,
                  ...
                  >
                    ...
                    >
                      Skip building vector search index (Index is required for
                      Semantic Explorer Search & Chat)
                    </label>
                  </div>
        ...
        {activeTab === "config" && (
          ...
          >
            <div>
              ...
              <p style={{ color: "var(--text-muted)" }}>
                Configure credentials, API base URLs, and active models for LLM
                and Embedder engines.
              </p>
            </div>
            ...
            {!configs ? (
              ...
              >
                  ...
                  >
                    ...
                    >
                      ...
                      <select
                        ...
                        onChange={(e) =>
                          updateLLM({ provider: e.target.value })
                        }
                      >
                    ...
                    >
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          updateLLM({ base_url: e.target.value })
                        }
                      />
                    </div>
                  </div>
                ...
                <div className="glass-panel" style={{ padding: "24px" }}>
                  <div className="card-header">
                    <h3 style={{ fontSize: "18px", color: "#ffb86c" }}>
                  ...
                  >
                    ...
                    >
                      ...
                      <select
                        value={configs.agent?.provider || "openai"}
                        onChange={(e) =>
                    ...
                    >
                      ...
                      <input
                        type="password"
                        placeholder="••••••••••••••••"
                        value={configs.agent?.api_key || ""}
                        onChange={(e) =>
                          updateAgent({ api_key: e.target.value })
                        }
                      />
                    </div>
                    ...
                    >
                      ...
                      <input
                        type="text"
                        placeholder="https://api.openai.com/v1"
                        value={configs.agent?.base_url || ""}
                        onChange={(e) =>
                          updateAgent({ base_url: e.target.value })
                        }
                      />
                    </div>
                  </div>
                ...
                <div className="glass-panel" style={{ padding: "24px" }}>
                  <div className="card-header">
                    ...
                    <span className="badge badge-secondary">
                      {configs.embedder.provider || "custom"}
                    </span>
                  </div>
                  ...
                  >
                    ...
                    >
                      ...
                      <select
                        ...
                        onChange={(e) =>
                          updateEmbedder({ provider: e.target.value })
                        }
                      >
                        <option value="openai">OpenAI</option>
                        <option value="bailian">Aliyun Bailian</option>
                    ...
                    >
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          updateEmbedder({ model: e.target.value })
                        }
                      />
                    </div>
                    ...
                    >
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          updateEmbedder({ api_key: e.target.value })
                        }
                      />
                    </div>
                    ...
                    >
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          updateEmbedder({ base_url: e.target.value })
                        }
                      />
                    </div>
                  </div>
                ...
                >
                  ⚙️ Settings are synchronized in real-time with the local CLI
                  config file at <code>~/.he/config.toml</code>.
                </div>
              </div>
            )}
          </div>
              ...
              >
                ...
                <div style={{ display: "flex", flexWrap: "wrap", gap: "6px" }}>
                  <span
                    ...
                    style={{ fontSize: "10px" }}
                  >
                    {activeKA.metadata?.template}
                  </span>
                  <span
                    className="badge badge-secondary"
                    style={{ fontSize: "10px" }}
                  >
                    {activeKA.metadata?.lang}
                  </span>
                  {activeKA.has_index && (
                    <span
              ...
              >
                ...
                <button
                  ...
                  style={{
                    flexGrow: 1,
                    background:
                      explorerTab === "chat" ? "var(--bg-card)" : "transparent",
                    color:
                      explorerTab === "chat" ? "#fff" : "var(--text-muted)",
                    padding: "8px 12px",
                    fontSize: "12px",
                  }}
                ...
                >
                  ...
                  >
                    {chatHistory.length === 0 ? (
                      ...
                      >
                        Ask questions about this Knowledge Abstract. The engine
                        will retrieve relevant nodes as local context.
                      </div>
                    ) : (
            ...
            >
              ...
              {explorerTab === "visualizer" && (
                ...
                >
                  ...
                  >
                    {activeKA.data.nodes?.length || 0} nodes ·{" "}
                    {activeKA.data.edges?.length || 0} edges
                  </div>
                        ...
                        >
                          <thead>
                            ...
                            >
                              ...
                              {activeKA.data.nodes?.[0]?.time && (
                                <th style={{ padding: "8px 12px" }}>Time</th>
                              )}
                              {activeKA.data.nodes?.[0]?.location && (
                                <th style={{ padding: "8px 12px" }}>
                                  Location
                                </th>
                              )}
              ...
              {agentChatHistory.length === 0 ? (
                ...
                >
                  ...
                  <div style={{ fontSize: "13px" }}>
                    I'm your AI agent. Ask me to orchestrate extractions or
